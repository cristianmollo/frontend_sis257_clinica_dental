...
<template>
  <div>
    <table>
      <thead>
        <tr>
          ...
          <th>Fecha y Hora de Suspensión</th>
          <th>Acciones</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(cita, index) in citasFiltradas" :key="cita.id">
          ...
          <td>
            {{ cita.fechaConfirmacion
              ? new Date(cita.fechaConfirmacion).toLocaleString()
              : 'No confirmado' }}
          </td>
          <td>
            {{ cita.fechaSuspension
              ? new Date(cita.fechaSuspension).toLocaleString()
              : 'No cancelado' }}
          </td>
          <!-- Botón para cambiar estado a Confirmado -->

          <td>
            <Button v-if="cita.estado === 'Pendiente'" icon="pi pi-check" label="Confirmar" text
              class="p-button-success" @click="confirmarCita(cita)" />
            <Button v-if="cita.estado === 'Pendiente'" icon="pi pi-check" label="Suspender" text class="p-button-danger"
    ...
    <div class="total-container" style="background-color: #f0f8ff; padding: 10px; border-radius: 5px; border: 2px solid #007bff;">
      <p style="font-size: 20px; font-weight: bold; color: #007bff;">
        Monto Total de Cobrar:
        <strong style="font-size: 24px; color: #ff4500;">{{ totalCobrar }} Bs.</strong>
      </p>
    </div>
