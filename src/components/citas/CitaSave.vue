...
function getServicioLabel(servicio: Servicios) {
  return servicio ? `${servicio.nombre} (${servicio.precio} Bs.)` : '';
}
...
async function handleSave() {
  try {
    ...
    if (
      new Date(cita.value.fechaHoraInicio).getTime() < fechaActual.getTime() ||
      new Date(cita.value.fechaHoraFin).getTime() < fechaActual.getTime()
    ) {
      toast.add({
        severity: 'warn',
        summary: 'Error',
        detail: 'Las fechas seleccionadas deben ser posteriores a la fecha y hora actual.',
        life: 5000,
      })
      return
    }
    ...
    if (error.response?.status === 409) {
      toast.add({
        severity: 'warn',
        summary: 'Error',
        detail: error.response.data?.message || 'Horario no disponible',
        life: 5000,
      })
    } else {
...
<template>
  <div class="card flex justify-center">
    <Dialog v-model:visible="dialogVisible" :header="props.modoEdicion ? 'Editar Cita' : 'Crear Cita'"
      style="width: 30rem">
      <div class="p-fluid">
        <!-- Odontólogo -->
        <div class="p-field mb-4">
          <label for="odontologo" class="font-semibold">Odontólogo</label>
          <Select id="odontologo" v-model="cita.odontologoId" :options="odontologos" :optionLabel="getOdontologoLabel"
            optionValue="id" placeholder="Seleccione un odontólogo" />
        </div>
        ...
        <div class="p-field mb-4">
          ...
          <DatePicker id="fechaInicio" v-model="cita.fechaHoraInicio" showIcon :showTime="true" dateFormat="yy-mm-dd"
            placeholder="Selecciona fecha y hora de inicio" :minDate="new Date()" />
        </div>

        <!-- Fecha de fin (solo lectura) -->
        <div class="p-field mb-4">
          <label for="fechaFin" class="font-semibold">Fecha Fin</label>
          <InputText id="fechaFin"
            :value="cita.fechaHoraFin ? new Date(cita.fechaHoraFin).toLocaleString() : 'Fecha no definida'" disabled />
        </div>
