...
const props = defineProps({
  usuarioLogeado: { type: Object, required: true }
})
      ...
      <table class="dentists-table">
        ...
        <tbody>
          <tr v-for="(odontologo, index) in odontologos" :key="odontologo.id">
            ...
            <td>
              <template v-if="props.usuarioLogeado && odontologo.id === props.usuarioLogeado.id">
              <div style="display: flex; gap: 0.5rem; justify-content: center;">
                <Button label="Ver servicios" icon="pi pi-eye" class="p-button-sm" @click="$emit('ver-servicios', odontologo)" />
                <Button label="Asignar" icon="pi pi-plus-circle" class="p-button-sm" @click="$emit('asignar-servicios', odontologo)" />
              </div>
              </template>
            </td>
